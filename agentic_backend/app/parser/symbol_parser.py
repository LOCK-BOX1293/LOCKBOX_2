...
class SymbolParser:
    def __init__(self) -> None:
        # Import symbols are high-volume and were flooding graph views.
        # Keep disabled by default until dedicated import-graph UX exists.
        self.include_import_symbols = (
            os.getenv("INDEX_IMPORT_SYMBOLS", "false").strip().lower() == "true"
        )
    ...
    def parse(
        ...
        # For non-code file types (markdown/yaml/json/etc.), avoid creating synthetic
        # module symbols that add graph noise without execution semantics.
        return []
    ...
    def _build_symbol(
        ...
        symbol_id = stable_hash(
            f"{repo_id}:{branch}:{commit_sha}:{file_path}:{symbol_type}:{name}:{start_line}:{end_line}"
        )
    ...
    def _parse_python(
        self, repo_id: str, branch: str, commit_sha: str, file_path: str, content: str
    ) -> list[dict]:
        symbols: list[dict] = []
        try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if self.include_import_symbols:
