...
export const nodeTypePalette: Record<NodeType, { label: string; accent: string; tone: string }> = {
...
export const toolInfo: Record<ToolId, string> = {
...
export function extractSegments(input: string): string[] {
  const rough = input
    .replace(/\n+/g, ' ')
...
export function detectNodeType(text: string): NodeType {
  const lower = text.toLowerCase();
  if (/[?]/.test(text) || /\b(why|how|what|when|where|who)\b/.test(lower)) return 'question';
  if (/\b(decide|choose|prefer|settle|confirm|adopt)\b/.test(lower)) return 'decision';
  if (/\b(do|build|ship|add|create|move|wire|write|review|make|implement)\b/.test(lower)) return 'step';
  if (/\b(search|source|evidence|paper|article|api|docs|reference|research)\b/.test(lower)) return 'search-result';
  return 'concept';
}

export function chooseTools(text: string, type: NodeType): ToolId[] {
  const lower = text.toLowerCase();
  const tools = new Set<ToolId>(['extract', 'promote']);
  if (/\b(search|api|docs|reference|research|evidence|article|source)\b/.test(lower)) tools.add('search');
  if (/\b(related|link|connect|same|similar|because|therefore|instead)\b/.test(lower)) tools.add('link');
  if (/\b(cluster|group|workspace|topic|theme)\b/.test(lower)) tools.add('cluster');
  if (type === 'decision') tools.add('link');
  return Array.from(tools);
}

export function topicLabel(input: string): string {
  const tokens = normalize(input);
  if (tokens.length === 0) return 'Untitled idea';
  return tokens.slice(0, 3).map((word) => word[0].toUpperCase() + word.slice(1)).join(' ');
}

export function workspaceCentroid(workspace: Workspace): string {
...
export function scoreDrift(activeTopic: string, input: string): number {
...
export function classifyEditIntent(input: string): 'add' | 'delete' | 'replace' | 'neutral' {
...
export function searchKnowledge(input: string): Array<Pick<PlannerNode, 'title' | 'detail' | 'type' | 'source'>> {
...
export function createNodePlacement(
  index: number,
  sourceNode?: PlannerNode | null,
  isolated = false,
  footprint: { width: number; height: number } = { width: 250, height: 92 },
) {
...
export function createEdge(from: string, to: string, kind: PlannerEdge['kind'] = 'related'): PlannerEdge {
  return {
    id: uid('edge'),
    from,
    to,
...
export function planTurn(params: {
  ...
  const action: WorkspaceAction = drift > 0.58 || driftPhrase ? 'split' : 'stay';
...
export function edgePath(from: PlannerNode, to: PlannerNode): string {
...
export function edgeLabelPosition(from: PlannerNode, to: PlannerNode) {
  const x1 = from.x + from.width;
  const x2 = to.x;
  return {
    x: (x1 + x2) / 2,
